...
public class LoginServlet extends HttpServlet {
    ...
    private final String VERIFY_AGAIN_PAGE = "verifyfail.html";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            TblUserDTO dto = dao.checkLogin(email, encrypted);

            if (dto != null) {
                //Check status in DB
                if (dto.getStatus() == 2) { //2 in DB means activate account successful
                    HttpSession session = request.getSession();
                    session.setAttribute("ACCOUNT", dto);

                    url = HOME_PAGE;
                } else if (dto.getStatus() == 1){ //1 in DB means new account
                    url = VERIFY_AGAIN_PAGE;
                }
            }
